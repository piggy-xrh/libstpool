...
/* Interfaces (@smlink_q_x) are just provided for user 
 * to operate the sets, our cache will not use them */
...
static inline void smlink_q_pushback(smlink_q_t *q, void *obj) {
	*(void **)obj = NULL;
	if (q->n)
		q->qlast->next = (struct smlink *)obj;
	else
		q->qfirst = (struct smlink *)obj;
	q->qlast = (struct smlink *)obj;
	++ q->n;
...
static inline void *smcache_get(smcache_t *smc, int create) {
	...
	smcache_lock(smc); 
	obj = smcache_getl(smc, CACHE_F_LOCK_CREATER & smc->lflags);	
	smcache_unlock(smc);
