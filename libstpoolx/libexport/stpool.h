...
typedef long (*Walk_cb)(struct stpool_tskstat_t *stat, void *arg);
...
EXPORT void   stpool_task_init(struct sttask_t *ptsk, 
					...
					void *arg);
/*@stpool_task_new
 *     Allocate a task object from the cache. (The task allocated by
 ...
 * 		@clone_schattr  [in] Tell the system whether it should clone the task's 
...
/* @stpool_mark_task_cb
 *	    Walk the tasks existing in the pool and mark them with specified 
 * flags returned by @wcb.
 *
 * Arguments:
 *    @hp      [in]  the pool handle 
 *
 *    @wcb     [in]  The callback to walk all tasks existing in the pool,
 *                   If it returns -1, @stpool_mark_task_cb will stop walking the tasks
 *                   and will return imediately. Or the return value will be treated as
 *                   the mark flags(STTASK_VMARK_XX) to mark the task passed to @wcb.
 *
 *                   see @stpool_mark_task for more details about the mark flags.
 *
 *    @wcb_arg [in] the argument reserved for @wcb
 *
 * Return:
 *    the number of effected tasks that processed by @wcb
*/
EXPORT int  stpool_mark_task_cb(HPOOL hp, Walk_cb wcb, void *wcb_arg);
...
/* @stpool_task_wait
 *	  Wait for the tasks' being free in @ms milliseconds. 
 *
 * Arguments:
 *    @hp           [in]  the pool handle 
 *
 *    @ptsk         [in]  If ptsk is not NULL, @stpool_task_wait will not return
 *                        until the task is free, Or @stpool_task_wait will wait 
 *                        for all tasks's being free.
 *
 *    @ms           [in]  -1 == INFINITE                   
 *                    
 * Return:
...
/* @stpool_task_wait_cb
 *	  Walk the task queue and wait for the theirs being free in @ms milliseconds 
 * if it is neccessary. 
 *    @stpool_task_wait_cb use the callback @wcb to walk the task. if @wcb returns
 * a none-zero value on a task, @stpool_task_wait_cb will wait for the task's being
 * free.
 *
 * Arguments:
 *    @hp           [in]  the pool handle 
 *
 *    @wcb          [in]  a callback to walk the task, if it returns a none-zero 
 *                        value on a task, @stpool_task_wait_cb will wait for the
 *                        task's being free.
 *
 *    @wcb_arg      [in]  the argument reserved for @wcb
 *
 ...
 *		(If it is woke up by @stpool_wakeup , it returns -1)
 */
EXPORT int  stpool_task_wait_cb(HPOOL hp, Walk_cb wcb, void *wcb_arg, long ms); 

/* @stpool_task_wait_entry
 ...
 *                          and @wait_all will be ignored. its function is equal to 
 *                          @stpool_task_wait(hp, NULL, ms).  the null item will be
 *                          skipped.
 *
 *    @n              [in]  the tasks item number of the @entry.
 *
 *    @wait_all       [in]  If @wait_all is equal to zero, it tell the @stpool_task_wait_entry 
 *                          that it should not return until the pool have done all of the tasks 
 *                          existing in the @entry. 
 *                          
 *                          Or @stpool_task_wait_entry will return imediately if one of the tasks
 *                          exitsting in the @entry is free. user can call @stpool_task_is_free to
 *                          check the task's status.
 *
 *    @ms             [in]  milliseconds to wait for tasks (-1 == INFINITE)
 *
 * Return:
 *	  On success, it returns 0.  
 *	  On timeout, 1 is returned, 
 *	 (If it is woke up by @stpool_wakeup , it returns -1)
 */
EXPORT int  stpool_task_wait_entry(HPOOL hp, struct sttask_t *entry[], int n, int wait_all, long ms);
