...
stpool_version() 
{
	return "2015/09/20-2.6.6-libstpool-FObjp-desc";
}
...
stpool_mark_task(HPOOL hp, struct sttask_t *ptsk, long lflags) 
{
	if (ptsk)
		...
		return tpool_mark_task_ex(hp, NULL, (void *)lflags);
}
...
EXPORT int  
stpool_task_any_wait(HPOOL hp, 
					struct sttask_t *entry[], 
					int n, 
					int *npre, 
					long ms) 
{
	return tpool_task_any_wait(hp, 
							(struct task_t **)entry, 
							n, 
							npre, 
							ms);
...
EXPORT int  
stpool_task_waitex(HPOOL hp, 
				   int (*sttask_match)(struct stpool_tskstat_t *, void *), 
				   void *arg, 
				   long ms) 
{
	return tpool_task_waitex(hp,
